...
const createNotificationSocket = async (user_id, socket_id) => {
  try {
    const data = await query(
      'SELECT socket_id FROM sockets WHERE user_id = $1',
      [user_id]
    )
    if (data.rows.length > 0) {
      await query(
        'UPDATE sockets SET socket_id = $1 WHERE user_id = $2',
        [socket_id, user_id]
      )
    } else {
      await query(
        'INSERT INTO sockets (user_id, socket_id) VALUES ($1, $2)',
        [user_id, socket_id]
      )
    }
  } catch (error) {
...
const deleteNotificationSocket = async (socket_id) => {
  try {
    const data = await query('DELETE FROM sockets WHERE socket_id = $1', [
      socket_id
    ])
